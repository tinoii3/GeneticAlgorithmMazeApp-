...
class HomePage(QWidget):
    ...
    def init_ui(self):
        ...
        logo_btn.setIcon(self.style().standardIcon(QStyle.SP_FileDialogListView))
        logo_btn.setIconSize(QSize(200, 200))
        logo_btn.setFlat(True)
        logo_btn.setStyleSheet("background-color: transparent;")
        
        # ชื่อแอป
        title_label = QLabel("MazePath Solver")
        ...
        # เมนูปุ่ม
        self.btn_create = QPushButton("Configuration Model", self)
        self.btn_create.setObjectName("menuBtn")
        self.btn_create.clicked.connect(lambda: self.parent.stacked_widget.setCurrentIndex(1))

        self.btn_guide = QPushButton("Guide", self)
        self.btn_guide.setObjectName("menuBtn")
        self.btn_guide.clicked.connect(lambda: self.parent.stacked_widget.setCurrentIndex(3))
        ...
        self.layout.addWidget(self.btn_create, alignment=Qt.AlignCenter)
